...
class XAIAdapter:
    ...
    def __init__(self, *, model: str, api_key: str | None = None, transport: JsonHttpTransport | None = None, base_url: str = "https://api.x.ai/v1", timeout: float = 60.0) -> None:
        if not model:
    ...
    def invoke(self, request: AdapterRequest) -> ProviderResponse:
        ...
        data = self.transport.post_json(
            ...
            payload={"model": self.model, "messages": [{"role": "user", "content": build_provider_prompt(request)}]},
